...
class SyncFolderClient:
    def __init__(self, config):
        ...
        if not self.sync_folder_path or not os.path.exists(self.sync_folder_path):
            raise ValueError("Sync folder not found. Please specify the full path in config.json using the 'sync_folder.path' setting.")
        ...
        for root, _, filenames in os.walk(folder):
            for name in filenames:
                files.append({
                    ...
                    'lastModifiedDateTime': os.path.getmtime(os.path.join(root, name))
                })
        return files
    ...
    def download_file(self, file_id, dest_path):
        ...
        if not os.path.isabs(file_id):
            candidate_paths = [
                os.path.join(self.encrypted_path, file_id),
                os.path.join(self.sync_folder_path, file_id)
            ]
            src = None
